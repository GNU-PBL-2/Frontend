...
function getLeftAccentClass(type: AlarmItem["type"], isRead: boolean) {
  ...
  switch (type) {
    case "expiry_warning": return "bg-red-400";
  ...
  return (
    <div
      className={`relative rounded-2xl border shadow-sm overflow-hidden transition-colors ${
        item.isRead ? "border-gray-100 bg-gray-50" : "border-gray-200 bg-white"
      }`}
    >
      <div className={`absolute left-0 top-0 h-full w-1 ${getLeftAccentClass(item.type, item.isRead)}`} />
      ...
      <div className="px-4 py-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className={`text-sm leading-6 break-words ${item.isRead ? "text-gray-400" : "text-gray-900"}`}>
              {item.message}
            </p>
